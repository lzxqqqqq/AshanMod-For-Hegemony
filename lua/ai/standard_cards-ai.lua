...
--【M&M6】相关
	if from:hasSkill("Mzengwu") and not from:hasSkills(sgs.slash_benefit_skill) and from:getHp() < 3 and getCardsNum("Jink", to, from) > 0 and to:getHandcardNum() > 2 then return false end  --【憎恶】
	...
	if from:getMark("@yaojibasic") == 1 and from:objectName() ~= to:objectName() then return false end  --【耀击】
	if from:getMark("@tiaozhan") == 1 and to:hasShownOneGeneral() then
		local kensei = self.room:findPlayerBySkillName("Mtiaozhan")
		if kensei and kensei:isAlive() and kensei:hasShownSkill("Mtiaozhan") and to:isFriendWith(kensei) and kensei:objectName() ~= to:objectName() then return false end
	end  --【挑战】
	if to:hasShownOneGeneral() then
		local ravager = self.room:findPlayerBySkillName("Mchaonong")
		if ravager and ravager:isAlive() and ravager:hasShownSkill("Mchaonong") and ravager:objectName() ~= to:objectName() and ravager:objectName() ~= from:objectName() then
			if ravager:isFriendWith(to) and getCardsNum("BasicCard", ravager, from) and from:inMyAttackRange(ravager) then
				return false
			end
		end
	end  --【嘲弄】
...
function SmartAI:useCardBefriendAttacking(BefriendAttacking, use)
	...
	for _, to_select in ipairs(players) do
		if self:isFriend(to_select) and BefriendAttacking:targetFilter(targets, to_select, self.player) and not targets:contains(to_select)
			...
			--【M&M】相关
			if to_select:getMark("@yinni") == 0 then
