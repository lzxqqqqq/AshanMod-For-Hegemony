...
function setInitialTables()
	...
	sgs.masochism_skill = 		"yiji|fankui|jieming|ganglie|fangzhu|hengjiang|qianhuan"..
							"|Mfanji|Mzhengyi|Mruoshui|Mdianxiao|Mdiyi|Mbaoxing|Mdianxiao|Mzhenue|Mheijing"--添加
	...
	sgs.drawpeach_skill =		"tuxi|qiaobian"..
							"|Mjiqu|Mshenpan|Mtansuo|Myouhuo|Mcaozong"--添加
	sgs.recover_skill =		"rende|kuanggu|zaiqi|jieyin|qingnang|yinghun|hunzi|shenzhi|buqu"..
							"|Mxuewo|Mxinyang|Mlianmin|Myanbo|Mniuqu|Mqianlin|Mzengzhi|Mshikong|Mshuangshen|Mcanyang|Myuanquan|Mrongjie"--添加
	sgs.use_lion_skill =		 "duanliang|qixi|guidao|lijian|zhiheng|fenxun|qingcheng"..
							"|Mshikong|Mshengyan|Mshouxue|Myanmo|Mbeici"--添加
	sgs.need_equip_skill = 		"shensu|beige|huyuan|qingcheng"..
							"|Mshikong|Mshenju|Mshengyan|Mshouxue|Myanmo|Mbeici|Mrongjie|Myuanshi"--添加
	sgs.judge_reason =		"bazhen|EightDiagram|supply_shortage|tuntian|qianxi|indulgence|lightning|leiji|tieqi|luoshen|ganglie"..
							"|Msiwang|Mliushi|Mxiezhi|Mshuailao|Mxinyang|Mshengyan|Mqianggong|Mkuchu"--添加
	sgs.slash_benefit_skill = "jiang|tieji|mengjin|liegong|wushuang|Mlangzhan|Mqianggong|Mzhaoya|Mbaoxing|Mfushi"  --难以闪避的杀或
	sgs.keep_card_skill = "keji|qiaobian|Myinji|Msuyuan|Mlangzhan|Mshenju"  --屯牌技能
	sgs.slash_use_skill = "paoxiao|Mtanlan|Mshennu|Mzhaoya"--连续使用杀的技能
...
--	global_room:writeToConsole(msg)    M&M 防止服务器刷新ai信息
...
function sgs.getDefense(player)
	...
	if player:hasShownSkills("yiji+qiaobiao|xiaoji+zhiheng|buqu+yinghun_sunjian|luoshen+guicai") then defense = defense + 1.5 end 
	...
	if positive then
		...
		elseif trick:isKindOf("Indulgence") then
			if self:isFriend(to) and not to:isSkipped(sgs.Player_Play) then
				if to:hasShownSkill("Mlingyun") then return nil end  --【M&M】相关
...
--【M&M】相关
	if from:hasSkill("Mchongfeng") and from:distanceTo(to) > 1 and slash then
		dmg = dmg + 1
	end  --【冲锋】
	local elrath
	for _,p in sgs.qlist(self.room:getOtherPlayers(from)) do
		if from:isFriendWith(p) and p:hasShownSkill("Mhuiyao") then
			elrath = p
			break
		end
	end
	if elrath and from:getHp() <= elrath:getHp() and elrath:inMyAttackRange(from) then
		dmg = dmg + 1
	end  --【辉耀】
	if from:hasSkill("Myinxue") and from:getMark("@yinxue") > 0 and slash then
		local x = from:getMark("@yinxue")
		dmg = dmg + x
	end  --【饮血】
	if from:hasSkill("Mzhaoya") and from:hasFlag("zhaoya") and slash then
		dmg = dmg + 1
	end  --【爪牙】
	if from:hasSkill("Mcanbao") and to:getHandcardNum() < 2 and slash then
		dmg = dmg + 1
	end  --【残暴】
	if from:hasSkill("Mbingbao") and to:hasEquip() and slash and not self:isFriend(from, to) then
		dmg = dmg + 1
	end  --【冰雹】
	if from:hasSkill("Mdianxiao") and from:getMark("@dianxiaoA") == 1 and slash then
		dmg = dmg + 1
	end  --【癫笑】
	if from:hasSkill("Mdiyi") and to:getMark("@diyi") == 1 and fireSlash then
		dmg = dmg + 1
	end  --【敌意】
	if to:hasShownSkill("Mxuewo") then
		dmg = 1
	end  --【血握】
	if to:hasShownSkill("Mjudun") and slash and not (fireSlash or thunderSlash) then
		dmg = 1
	end  --【巨盾】
...
function SmartAI:hasCrossbowEffect(player)
	player = player or self.player--【M&M】相关
	return player:hasWeapon("Crossbow") or player:hasShownSkill("paoxiao") or player:hasShownSkill("Mtanlan") or player:hasShownSkill("Mjingjie")
end
...
--			self.room:writeToConsole("Pindian card for " .. reason .. " not found!!")  M&M 去除拼点时的无用提示
...
--【M&M】相关
	if from then
		if to:hasShownSkill("Mmingyun") and from:isKongcheng() then return false end --【命运】
		local elrath
		for _,p in sgs.qlist(self.room:getOtherPlayers(to)) do
			if to:isFriendWith(p) and p:hasShownSkill("Mhuiyao") then
				elrath = p
				break
			end
		end
		if elrath and to:getHp() <= elrath:getHp() and elrath:inMyAttackRange(to) then
			damage = damage - 1
			if damage < 1 then return false end
		end --【辉耀】
		if from:getMark("@tiaozhan") == 1 then
			local kensei = self.room:findPlayerBySkillName("Mtiaozhan")
			if kensei and kensei:isAlive() and kensei:hasShownSkill("Mtiaozhan") and kensei:objectName() ~= to:objectName() then
				damage = damage - 1
				if damage < 1 then return false end
			end
		end  --【挑战】
		if to:hasSkill("Myiwang") and to:hasShownSkill("Myiwang") and not to:isKongcheng() and to:getHp() <= 1 then
			if not (from:hasWeapon("ice_sword") or (from:hasWeapon("kylin_bow") and (to:getDefensiveHorse() or to:getOffensiveHorse()))) then
				return false
			end
		end  --【遗忘】
		if to:hasSkill("Mdiyi") and from:getMark("@diyi") == 1 then return false end  --【敌意】
		if (to:getMark("chaomu") == 1 or (to:isFriendWith(from) and to:hasShownOneGeneral() and to:objectName() ~= from:objectName())) and from:hasSkill("Mshimo") then return false end  --【噬魔】
	end
...
--【M&M】相关
	if attacker:hasShownOneGeneral() and (card:isKindOf("SavageAssault") or card:isKindOf("ArcheryAttack")) and attacker:aliveCount() > 2 then
		local shalassa
		for _,p in sgs.qlist(self.room:getAlivePlayers()) do
			if p:hasShownSkill("Mbolan") and p:getRole() ~= "careerist" then
				shalassa = p
				break
			end
		end
		if shalassa and shalassa:isAlive() and attacker:isFriendWith(shalassa) then
			return 10
		end
	end
--结束
...
--【M&M】相关
	if to:hasShownSkill("Mmingyun") and not (card:isKindOf("FireAttack") or card:isVirtualCard()) then return false end  --【命运】
	if to:hasShownSkill("Mwuxing") and (card:isKindOf("SavageAssault") or card:isKindOf("ArcheryAttack")) then
		local current = self.room:getCurrent()
		if current and current:isAlive() and current:getHandcardNum() >= to:getHandcardNum() then
			return false
		end
	end  --【无形】
	if self:isWeak(from) and to:hasShownOneGeneral() then
		local torian =  self.room:findPlayerBySkillName("Mshouwei")
		if torian and torian:isAlive() and torian:hasShownSkill("Mshouwei") and torian:objectName() ~= to:objectName() and not self:isWeak(torian) and self:isFriend(torian, to) and not self:isFriend(torian, from) and torian:inMyAttackRange(from) and not (from:hasArmorEffect("Vine") and not torian:hasWeapon("Fan"))then
			if card:isKindOf("Duel") then
				return false
			end
		end
	end  -- 【守卫】
	if to:hasShownSkill("Mfuchong") and card:isKindOf("Duel") then return false end  --【俯冲】
	if to:hasSkill("Myiwang") and to:hasShownSkill("Myiwang") and not to:isKongcheng() and to:getHp() < 2 and (card:isKindOf("Duel") or card:isKindOf("FireAttack") or card:isKindOf("SavageAssault") or card:isKindOf("ArcheryAttack")) then
		return false
	end  --【遗忘】
	if to:hasShownSkill("Mbingjing") and card:isKindOf("TrickCard") and card:getSuit() == sgs.Card_Spade and not card:isNDTrick() and not to:isKongcheng() then return false end  --【冰晶】
	if to:hasShownSkill("Mmoxing") and from:getGender() ~= to:getGender() and card:isRed() and card:isKindOf("TrickCard") then return false end --【魔性】
	if to:hasShownOneGeneral() and from:getMark("@tiaozhan") == 1 and (card:isKindOf("Duel") or card:isKindOf("FireAttack")) then
		local kensei = self.room:findPlayerBySkillName("Mtiaozhan")
		if kensei and kensei:isAlive() and kensei:hasShownSkill("Mtiaozhan") and kensei:isFriendWith(to) and kensei:objectName() ~= to:objectName() then
			return false
		end
	end  --【挑战】
	if from:hasSkill("Mlingbo") and card:isKindOf("Lightning") then return false end  --【凌波】
	if to:hasShownSkill("Mlingyun") and (card:isKindOf("Indulgence") or card:isKindOf("SupplyShortage")) then return false end  --【凌云】
	if to:hasShownSkill("Mlingyun") and from:objectName() ~= to:objectName() and card:isKindOf("TrickCard") and card:isNDTrick() then return false end  --【凌云】
	if from:hasShownOneGeneral() and to:hasShownOneGeneral() and card:isKindOf("GodSalvation") or card:isKindOf("AmazingGrace") and from:aliveCount() > 1 then
		local shalassa
		for _,p in sgs.qlist(self.room:getOtherPlayers(from)) do
			if p:hasSkill("Mbolan") and p:hasShownSkill("Mbolan") and p:getRole() ~= "careerist" then
				shalassa = p
				break
			end
		end
		if shalassa and from:isFriendWith(shalassa) and not to:isFriendWith(shalassa) then
			return false
		end
	end
	if from:hasShownOneGeneral() and to:hasShownOneGeneral() and (card:isKindOf("SavageAssault") or card:isKindOf("ArcheryAttack")) and from:aliveCount() > 2 then
		local shalassa
		for _,p in sgs.qlist(self.room:getOtherPlayers(from)) do
			if p:hasShownSkill("Mbolan") and p:getRole() ~= "careerist" then
				shalassa = p
		...
		if shalassa and shalassa:isAlive() and from:isFriendWith(shalassa) and to:isFriendWith(shalassa) then
			return false
		end
	end  --【波澜】
	if to:hasShownSkill("Mniuqu") and to:isWounded() and card:isRed() then
		if self:isFriend(from, to) then
			if not (card:isKindOf("GodSalvation") or card:isKindOf("AmazingGrace") or card:isKindOf("BefriendAttacking") or card:isKindOf("IronChain")) then
				return false
			end
		else
			if not (card:isKindOf("GodSalvation") or card:isKindOf("AmazingGrace")) then
				return false
			end
		end
	end  --【扭曲】
	if from:hasShownOneGeneral() and to:hasShownSkill("Mzhemo") and not to:isAllNude() and not from:isFriendWith(to) and card:isNDTrick() then
		if to:isKongcheng() and not to:hasEquip() then
			if self:isFriend(from, to) then
				if not (card:isKindOf("GodSalvation") or card:isKindOf("AmazingGrace") or card:isKindOf("BefriendAttacking") or card:isKindOf("IronChain")) then
					return false
				end
			...
			if self:isFriend(from, to) then
				if card:isKindOf("GodSalvation") or card:isKindOf("AmazingGrace") or card:isKindOf("BefriendAttacking") or card:isKindOf("IronChain") then
					return false
				end
			else
				if to:getHandcardNum() > 2 and to:inMyAttackRange(from) then
					if not (card:isKindOf("GodSalvation") or card:isKindOf("AmazingGrace")) then
						return false
					end
				end
	...
	if to:hasSkill("Mshinue") and not to:isAllNude() and from:objectName() ~= to:objectName() and card:isNDTrick() then
		if to:isKongcheng() and not to:hasEquip() then
			if self:isFriend(from, to) then
				if not (card:isKindOf("GodSalvation") or card:isKindOf("AmazingGrace") or card:isKindOf("BefriendAttacking") or card:isKindOf("IronChain")) then
					return false
			...
			if self:isFriend(from, to) then
				...
				if card:isKindOf("GodSalvation") or card:isKindOf("AmazingGrace") then
					return false
				end
			end
		end
	end  --【施虐】
	if to:hasShownSkill("Mdiyi") and from:getMark("@diyi") == 1 then
		if card:isKindOf("SavageAssault") or card:isKindOf("ArcheryAttack") or card:isKindOf("Duel") or card:isKindOf("FireAttack") then
			return false
		end
	end  --【敌意】
	if to:hasShownSkill("Mlonglin") and card:isNDTrick() and not (card:isKindOf("Dismantlement") or card:isKindOf("Snatch")) then
		return false
	end  --【龙鳞】
	if to:hasShownSkill("Mbeici") and from:objectName() ~= to:objectName() and to:getMark("@yinni") > 0 and (card:isKindOf("Duel") or card:isKindOf("Dismantlement") or card:isKindOf("Snatch") or card:isKindOf("FireAttack") or card:isKindOf("BefriendAttacking") or card:isKindOf("KnownBoth")) then
		return false
	end  --【背刺】
