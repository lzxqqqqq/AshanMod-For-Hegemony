...
void ServerPlayer::showGeneral(bool head_general, bool trigger_event, bool sendLog) {
    ...
    if (head_general) {
        ...
        if (!hasShownGeneral2()) {
            ...
            int i = 1;
            bool has_lord = isAlive() && (getGeneral()->isLord() || getMark("@zhengfeng_lord") > 0); // 争锋模式的领主决定
            if (!has_lord) {
                foreach(ServerPlayer *p, room->getOtherPlayers(this, true)) {
                    if (p->getKingdom() == kingdom) {
                        if ((p->getGeneral()->isLord() || p->getMark("@zhengfeng_lord") > 0) && p->getRole() != "careerist") { // 争锋模式中防止错误的出现野心家
                            has_lord = true;
                            break;
                        }
			...
			if ((!has_lord && i > (room->getPlayers().length() / 2)) || (getMark("zhengfeng_careerist") > 0) || (has_lord && getLord(true)->isDead() && getLord(true)->getRole() != "careerist")) { // 争锋模式中防止错误的出现野心家
